...
def calcular_cambio():
    try:
        ...
        if monto_pagado < monto_compra:
            ...
            resultado.set(f"El cambio a devolver es: Q{cambio:.2f}")
    except ValueError:
...
tk.Label(ventana, text="Monto de Compra: Q").grid(row=0, column=0, padx=10, pady=5)
entry_compra = tk.Entry(ventana)
entry_compra.grid(row=0, column=1, padx=10, pady=5)

tk.Label(ventana, text="Monto Pagado: Q").grid(row=1, column=0, padx=10, pady=5)
